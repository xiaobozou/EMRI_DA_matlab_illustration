...
for id_u = -10:10
    for id_v = -10:10  
        ...
        phi0 = 2.0/3.0*Phi_plane + 1.0/(3.0*sqrt(2))*id_u*delta + 1.0/sqrt(2.0)*id_v*delta;
        gam0 = 2.0/3.0*Phi_plane + 1.0/(3.0*sqrt(2))*id_u*delta - 1.0/sqrt(2.0)*id_v*delta;
        alp0 =-1.0/3.0*Phi_plane + 4.0/(3.0*sqrt(2))*id_u*delta + 0.0/sqrt(2.0)*id_v*delta;
        ...
        LLR_2D(id_u+11,id_v+11) = sqrt(LLR2_TD);
    end
end
...
v = (-10:10)*delta;
[U,V] = meshgrid(u,v);
save('./Results/LLR_3D_intial_phase.mat','LLR_2D','U','V');
figure(1)
pcolor(U,V,LLR_2D)
shading interp
set(gca,'fontsize',20);
...
sgt = sgtitle('Test 3D sub space ($\phi_0,\tilde{\gamma}_0,\alpha_0$), 2026/04/23','Color','k','FontSize',30,'Interpreter', 'latex');
saveas(gcf,'./Figures/test7_3D_landscape.png')
