function [hpSSB,hcSSB]=func_get_HAK(theta_s,phi_s,theta_k,phi_k,ut,Mt,aa,lam,Dt,phit,nut,gamt,et,alpt,num_harmonics)
...
%% harmonics summation
%// 2022/01/10
%// use top 10 harmonics
%// pick up from LDC params
%// actually it is not generally enough
%// need test more params, near and far from LDC params
%// snr  36.1436   29.4640   15.3486   12.8778   12.0768   11.1941    5.5367    5.4016    5.1060    4.6604
%// id  7     8    12     6    13     9    10    11    17    14
%// n   2     2    3      2    3      2     2     3    4     3  
%// m  -1     0    -1     -2   0      1     2     -2   -1    1

%// idex in c
%// i   1     1    2      1    2      1     1     2    3     2
%// j   1     2    1      0    2      3     4     0    1     3

%// idex in matlab
%// i   2     2    3      2    3      2     2     3    4     3
%// j   2     3    2      1    3      4     5     1    2     4
id_top10_harmonics = [2,2; ...
                    2,3; ...
                    3,2; ...
                    2,1; ...
                    3,3; ...
...
if num_harmonics == 25
    for i=1:25
        ...
        hpS = hpS -0.5*real(Amp_nt*Ap(m));
        hcS = hcS -1.0*real(Amp_nt*Ac(m));
        
    end
elseif num_harmonics==10 | num_harmonics==5 | num_harmonics==3
    for i=1:num_harmonics
        n = id_top10_harmonics(i,1);
        m = id_top10_harmonics(i,2);
    ...
    fprintf("only 25,10,5,3 harmonics summation is supported, check your num_harmonics....\n");
end   
time = toc;
fprintf("for %d harmonics summation, HAK hpS,hcS take %f sec\n",num_harmonics,time);
...
clear hpS;
clear hcS;
%save("0105_HAK_ode_deltat_interp_dt_m.mat",'tVecl','hpSSB','hcSSB');
end
