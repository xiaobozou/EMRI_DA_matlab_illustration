...
% save PSO history
PSO_dump = zeros(maxSteps,nDim+1);
for lpc_steps=1:maxSteps  
    ...
    if best_in_history > best_fitness
         ...
         % 2026/01/08 save PSO history
         PSO_dump(lpc_steps,1:nDim) = startCoord;
         PSO_dump(lpc_steps,1+nDim) = best_in_history;
    ...
    %     best_in_history = endFitnessVal;
        ...
        PSO_dump(lpc_steps,1:nDim) = pop(bestfitParticle,partCoordCols);
        PSO_dump(lpc_steps,1+nDim) = best_fitness;
    end
    for k = 1:popsize
           switch k
               case 1
