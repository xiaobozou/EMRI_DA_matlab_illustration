...
if  start_ok == 1   
    ...
    % 25 harmonics
    num_harmonics = 25; 
    hp_sum = zeros(1,N_fft);
    hc_sum = zeros(1,N_fft);
    for id = 1:25
        n = floor((id-1)/5) +1;
        m = mod(id-1,5) +1;
...
M = corrcoef(hp2,hp_sum);
fprintf('corr hp = %f\n',M(1,2));

M = corrcoef(hc2,hc_sum);
fprintf('corr hc = %f\n',M(1,2));
...
figure
subplot(2,2,1)
plot(tVecl(1:N_fft),hp_sum, tVecl(1:N_fft),hp_sum-hp2)
set(gca,'fontsize',20);
xlabel('time(sec)')
ylabel('hp')
legend('full hp','full hp diff')
subplot(2,2,2)
plot(tVecl(1:N_fft),hc_sum, tVecl(1:N_fft),hc_sum-hc2)
set(gca,'fontsize',20);
xlabel('time(sec)')
ylabel('hc')
legend('full hc','full hc diff')
subplot(2,2,3)
plot(tVecl(segment),hp_sum(segment),tVecl(segment),hp_sum(segment)-hp2(segment))
set(gca,'fontsize',20);
xlabel('time(sec)')
ylabel('hp')
legend('segment hp','segment hp diff')
subplot(2,2,4)
plot(tVecl(segment),hc_sum(segment),tVecl(segment),hc_sum(segment)-hc2(segment))
set(gca,'fontsize',20);
xlabel('time(sec)')
ylabel('hc')
legend('segment hc','segment hc diff')
screenSize = get(0, 'ScreenSize'); % Get screen resolution [1,4](@ref)  
set(gcf, 'OuterPosition', screenSize); % Cover entire screen  
sgt = sgtitle('Test AK waveform via harmonics summation, 2026/04/23','Color','k','FontSize',30,'Interpreter', 'latex');
saveas(gcf,'./Figures/test_1_2.png')